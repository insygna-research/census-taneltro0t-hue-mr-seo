...
def device_flow():
    ...
    try:
        ...
        print(json.dumps({"ok": False, "error": safe_exception(exc)}, ensure_ascii=False))
        return
    if "device_code" not in r:
        print(json.dumps({"ok": False, "error": redact_secrets(r)[:200]}, ensure_ascii=False))
        return
    ...
    while time.time() < deadline:
        ...
        if "access_token" in t:
            atomic_write_private(
                TOKENS_FILE, json.dumps(t, ensure_ascii=False, indent=2)
            )
...
def refresh():
    ...
    if "access_token" in t:
        ...
        print(json.dumps({"ok": False, "error": redact_secrets(t)[:200]}, ensure_ascii=False))
